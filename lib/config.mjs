...
// ---- Bankroll / ledger ----
// All risk limits below are FRACTIONS of current bankroll, so they scale with
// this number automatically — same principles, smaller base.
export const LEDGER = {
...
export const SIGNAL = {
  ...
  gate: 80,              // MIN signal quality to trade. Raised from 70 to tilt
                         // further toward CONFIDENCE — a play must be high-quality
                         // (tight book agreement, reliable sources, liquid, fresh)
                         // AND clear the net-EV gate. Sits alongside net-EV, not
                         // as a stand-in for it.
};

// ---- Trade timing window (WHEN, relative to kickoff, a bet is worth placing) --
// Betting weeks out is inefficient: book lines are still soft and noisy, Kalshi
// game markets are illiquid or not yet posted, and lineup/injury news hasn't
// landed — so a "fair value" that far out is unreliable and the edge is likely
// noise. Betting too late leaves no time to act. So only games kicking off
// within [minHoursToStart, maxHoursToStart] are eligible. Tune via env to find
// your happy medium (e.g. TIMING_MAX_HOURS=48 for tighter, 120 for looser).
export const TIMING = {
  minHoursToStart: +(process.env.TIMING_MIN_HOURS ?? 0),   // skip games already started / within this many hours of kickoff.
  maxHoursToStart: +(process.env.TIMING_MAX_HOURS ?? 72),  // skip games more than this many hours away (default 3 days).
};
...
export const GATE = {
  ...
  maxPlausibleEdgePts: 0.25, // > 25 pts raw edge is almost always a data error
                             // (stale line, mismatched/illiquid market), NOT free
                             // money — reject as suspicious. (SIM offsets max ~14pts.)
