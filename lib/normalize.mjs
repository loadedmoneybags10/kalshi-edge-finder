...
export function booksBlock(event, marketKey, sideOf) {
  const out = {};
...
// ============================ NFL ============================
// Moneyline (h2h) + point spread (variable line) + points O/U. a = away, b = home.
export const nflId = (away, home) => `nfl:${slug(away)}-${slug(home)}`;
export function buildNflFixture(event, k) {
  const away = event.away_team, home = event.home_team;
  const spLine = k.sp.line, totalLine = k.ou.line;
  const favName = k.sp.favSide === "a" ? away : home, dogName = k.sp.favSide === "a" ? home : away;
  return {
    id: nflId(away, home), sport: "nfl", league: "nfl", name: `${away} vs. ${home}`,
    slot: `${event.commence_time?.slice(0, 10) || ""} · ${home}`, sideA: away, sideB: home,
    commenceTime: event.commence_time || null,
    markets: [
      { key: "ml", label: "Moneyline", type: "winner", a: away, b: home,
        kalshi: twoWayKalshi(k.ml.yes, k.ml.yesSide),
        books: booksBlock(event, "h2h", oc => (oc.name === away ? "a" : oc.name === home ? "b" : null)) },
      { key: "sp", label: `Spread ${spLine}`, type: "spread", line: spLine, favSide: k.sp.favSide,
        a: `${favName} -${spLine}`, b: `${dogName} +${spLine}`,
        kalshi: twoWayKalshi(k.sp.yes, k.sp.yesSide),
        books: booksBlock(event, "spreads", oc => (oc.point < 0 ? "a" : oc.point > 0 ? "b" : null)) },
      { key: "ou", label: `Points O/U ${totalLine}`, type: "points_ou", line: totalLine,
        a: `Over ${totalLine}`, b: `Under ${totalLine}`,
        kalshi: twoWayKalshi(k.ou.yes, k.ou.yesSide),
        books: booksBlock(event, "totals", oc => (oc.name === "Over" ? "a" : oc.name === "Under" ? "b" : null)) },
    ],
...
export function nflResultFromScore(ev) {
  const s = Object.fromEntries((ev.scores || []).map(x => [x.name, +x.score]));
  const as = s[ev.away_team], hs = s[ev.home_team];
  if (as == null || hs == null || Number.isNaN(as) || Number.isNaN(hs)) return null;
  return { winner: as > hs ? "a" : "b", margin: Math.abs(as - hs), totalPoints: as + hs };
}
...
export function buildSoccerFixture(event, k, league = "mls") {
  ...
  // BTTS only when we actually have a Kalshi quote for it (real books rarely
  // offer BTTS, so SIM mode omits it rather than fabricating a market).
  if (k.btts) markets.push({ key: "btts", label: "Both Teams to Score", type: "btts", a: "BTTS: Yes", b: "BTTS: No",
  ...
  return {
    ...
    sideA: home, sideB: away, commenceTime: event.commence_time || null, markets,
  };
}
