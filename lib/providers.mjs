...
// AUTO-DEFAULT: if an ODDS_API_KEY is present we default to sim (real odds) —
// so you don't have to set FEED at all. Explicit FEED=... always wins;
// USE_MOCK=0 also forces sim. No key → mock.
export const FEED = (process.env.FEED
  || (ODDS_API_KEY ? "sim" : (process.env.USE_MOCK === "0" ? "sim" : "mock"))).toLowerCase();
...
export async function fetchActiveSports() {
  if (MOCK || !ODDS_API_KEY) return null;
  try {
