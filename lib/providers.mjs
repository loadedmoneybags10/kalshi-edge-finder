...
// fetch with an abort timeout — a stalled Odds API call fails loudly rather than
// hanging the poll. Override with ODDS_TIMEOUT_MS.
const ODDS_TIMEOUT_MS = +(process.env.ODDS_TIMEOUT_MS || 20000);
async function fetchT(url, opts = {}, ms = ODDS_TIMEOUT_MS) {
...
// Feed mode (evaluated lazily so a runner can set process.env.FEED at startup):
//   mock — everything simulated (no keys needed).
//   sim  — REAL book odds + REAL scores (The Odds API) + SIMULATED Kalshi price.
//   live — REAL book odds + REAL Kalshi prices (matched by team name).
// AUTO-DEFAULT: with an ODDS_API_KEY present and no FEED set → sim. No key → mock.
export function feedMode() {
  return (process.env.FEED
    || (process.env.ODDS_API_KEY ? "sim" : (process.env.USE_MOCK === "0" ? "sim" : "mock"))).toLowerCase();
}
const isMock = () => feedMode() === "mock";

const KALSHI_BASE = process.env.KALSHI_BASE || "https://api.elections.kalshi.com/trade-api/v2";
// Markets to request. Cost = (#markets × #regions) credits per call. Trim to
// "h2h" to roughly third the credit burn (moneyline/winner only).
const ODDS_MARKETS = process.env.ODDS_MARKETS || "h2h,totals,spreads";
// Regions to request. `eu` is included so we get PINNACLE — the sharpest book in
// the world and the best available proxy for true probability (it's NOT in the
// `us` region). Cost = (#markets × #regions) credits per call, so us,eu doubles
// the burn vs us-only; override with ODDS_REGIONS=us to save credits.
const ODDS_REGIONS = process.env.ODDS_REGIONS || "us,eu";

// Engine bucket (mlb | mls | ufc) for a competition key.
export const bucketOf = key => COMPETITIONS[key]?.sport || key;

// Credit meter: the-odds-api returns remaining/used in response headers. We keep
// the latest so the poller can report burn without extra calls.
let lastCredits = { remaining: null, used: null };
export const oddsCredits = () => ({ ...lastCredits });

// FREE (0-credit) call: which sports are in season right now. Lets the poller
// skip out-of-season leagues instead of paying to learn they're empty.
// Returns a Set of active Odds API sport keys, or null (mock / unknown → scan all).
export async function fetchActiveSports() {
  if (isMock() || !ODDS_API_KEY) return null;
  try {
    const res = await fetchT(`https://api.the-odds-api.com/v4/sports/?apiKey=${ODDS_API_KEY}`);
    if (!res.ok) return null;
    const arr = await res.json();
    return new Set((Array.isArray(arr) ? arr : []).filter(s => s.active).map(s => s.key));
  } catch { return null; }
}

// ---- The Odds API — https://the-odds-api.com/liveapi/guides/v4/ ----
export async function fetchBookOdds(compKey) {
  if (isMock()) return mockOdds(compKey);
  if (!ODDS_API_KEY) throw new Error("ODDS_API_KEY missing (set it, or FEED=mock)");
  const url =
    `https://api.the-odds-api.com/v4/sports/${oddsSportFor(compKey)}/odds` +
    `?apiKey=${ODDS_API_KEY}&regions=${ODDS_REGIONS}&markets=${ODDS_MARKETS}` +
  ...
  if (rem != null) lastCredits = { remaining: +rem, used: used != null ? +used : lastCredits.used };
  return res.json();
}

export const oddsConfig = () => ({ markets: ODDS_MARKETS, regions: ODDS_REGIONS, creditsPerCall: ODDS_MARKETS.split(",").length * ODDS_REGIONS.split(",").length });

// ---- Kalshi Trade API v2 (public market data; unauthenticated GET) ----
export async function fetchKalshiMarkets(compKey) {
  if (isMock()) return mockKalshi(compKey);
...
export async function fetchScores(compKey) {
  if (isMock()) return mockScores(compKey);
...
// All open Kalshi markets (public, no key) for team-based matching in live mode.
// mock → []. Returns the raw market objects (cents prices) for lib/match.mjs.
export async function fetchKalshiOpenMarkets() {
  if (isMock()) return [];
  const { listAllMarkets } = await import("./kalshi.mjs");
  const { markets } = await listAllMarkets(KALSHI_BASE, { maxPages: 20, log: true });
  return markets;
}

// Open markets for ONE competition's single-game series (KXMLBGAME, etc.) — the
// efficient, correct way to get real game moneylines (the bulk feed is flooded
// by the KXMVECROSSCATEGORY parlay series). mock or no series → []. Paginated.
export async function fetchKalshiGameMarkets(compKey) {
  if (isMock()) return [];
  const series = COMPETITIONS[compKey]?.kalshiSeries;
  if (!series) return [];
  const { kalshiGet } = await import("./kalshi.mjs");
  const out = []; let cursor = "";
  for (let i = 0; i < 6; i++) {
    const r = await kalshiGet(`/markets?series_ticker=${encodeURIComponent(series)}&status=open&limit=1000${cursor ? `&cursor=${encodeURIComponent(cursor)}` : ""}`, KALSHI_BASE);
    if (!r.ok || !r.json) break;
    for (const m of r.json.markets || []) out.push(m);
    cursor = r.json.cursor || ""; if (!cursor) break;
  }
  return out;
}

// Live Odds API check (uses the FREE /sports endpoint, 0 credits). Used by
// /api/health to confirm the key actually works.
export async function oddsSelfTest() {
  if (!ODDS_API_KEY) return { keyPresent: false, note: "ODDS_API_KEY not set" };
  try {
    const r = await fetchT(`https://api.the-odds-api.com/v4/sports/?apiKey=${ODDS_API_KEY}`);
    const rem = r.headers.get("x-requests-remaining");
    if (!r.ok) return { keyPresent: true, ok: false, status: r.status, body: (await r.text()).slice(0, 200) };
    const arr = await r.json();
    return { keyPresent: true, ok: true, status: r.status,
      activeSports: Array.isArray(arr) ? arr.filter(s => s.active).length : 0, creditsRemaining: rem };
  } catch (e) { return { keyPresent: true, ok: false, error: String(e?.message || e) }; }
}
