...
// fetch with an abort timeout so a stalled request fails loudly instead of
// hanging the whole poll forever. Override with KALSHI_TIMEOUT_MS.
const TIMEOUT_MS = +(process.env.KALSHI_TIMEOUT_MS || 20000);
...
export async function kalshiGet(path, base = KALSHI_BASE) {
  ...
  let res;
  try { res = await fetchT(url, { headers: { Accept: "application/json" } }); }
  catch (e) { return { ok: false, status: 0, json: null, textSample: `fetch failed: ${String(e?.name === "AbortError" ? `timeout after ${TIMEOUT_MS}ms` : e?.message || e)}`, url }; }
...
export async function listAllMarkets(base, { maxPages = 12, pageSize = 1000, status = "open", log = false } = {}) {
  const markets = [];
  let cursor = "", pages = 0, lastStatus = 0;
  for (; pages < maxPages; pages++) {
    const r = await listMarkets(base, { cursor, limit: pageSize, status });
    lastStatus = r.status;
    if (log) process.stderr.write(`\r   Kalshi markets: page ${pages + 1}/${maxPages}, ${markets.length} loaded…   `);
    if (!r.ok || !r.json) { if (log) process.stderr.write(`\n   ⚠ Kalshi page ${pages + 1} failed (status ${r.status}): ${r.textSample || ""}\n`); break; }
    for (const m of r.json.markets || []) markets.push(m);
    cursor = r.json.cursor || "";
    if (!cursor) break;
  }
  if (log) process.stderr.write(`\n   Kalshi markets: ${markets.length} total across ${pages + 1} page(s).\n`);
