...
export const REJECT = {
  ...
  SUSPICIOUS_EDGE: "edge implausibly large — likely stale/mismatched/illiquid data, not a real mispricing",
...
const MARKET_FAMILY = {
  ...
  runs_ou: "total", goals_ou: "total", rounds: "total", totals: "total", points_ou: "total",
...
function applyGate(d) {
  ...
  if (d.probEdgePts > GATE.maxPlausibleEdgePts) return set("SUSPICIOUS_EDGE"); // too good to be true → data error
...
export function initLedger(ids = agentIds(), st = START) {
  ...
  return {
    ...
    candidates: [], rejections: [], oddsCache: {}, schemaVersion: 2,
  };
}
...
function commit(st, a, d, stake) {
  ...
  const pos = {
    ...
    fixtureName: d.fixtureName, fightName: d.fixtureName, commenceTime: d.commenceTime ?? null,
...
function resolveSide(mk, res) {
  ...
  if (mk.type === "points_ou") return res.totalPoints == null ? null : res.totalPoints > mk.line ? "a" : "b";
  if (mk.type === "runline") { if (!res.winner || res.margin == null) return null; return res.winner === mk.favSide && res.margin >= 2 ? "a" : "b"; }
  if (mk.type === "spread") { if (!res.winner || res.margin == null) return null; return res.winner === mk.favSide && res.margin > mk.line ? "a" : "b"; }
  return null;
}
