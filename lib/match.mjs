...
// Price block for Kalshi's CURRENT market shape: prices arrive as *_dollars
// STRINGS already in 0–1 (e.g. "0.4500"), volume as *_fp. (Legacy cents fields
// are gone; centsBlock above is kept for the older mock/sim shapes.)
export const dollarsBlock = m => {
  const num = v => (v == null || v === "" ? null : Number(v));
  const ask = num(m.yes_ask_dollars) ?? num(m.last_price_dollars);
  const bid = num(m.yes_bid_dollars) ?? ask;
  const last = num(m.last_price_dollars) ?? ask;
  const vol = num(m.volume_fp) ?? num(m.volume_24h_fp) ?? 0;
  return { ask, bid, last, volume: vol || 0 };
};

// Parse the GAME DATE (UTC midnight) from a Kalshi game ticker's middle segment,
// e.g. KXMLBGAME-26SEP152140MIAAZ-MIA -> 2026-09-15, KXNFLGAME-26SEP21NYGLAR -> 2026-09-21.
// A market's `close_time` is a PADDED expiration (often days after the game), so
// it's unreliable for matching — the ticker date is authoritative.
const MON = { JAN: 0, FEB: 1, MAR: 2, APR: 3, MAY: 4, JUN: 5, JUL: 6, AUG: 7, SEP: 8, OCT: 9, NOV: 10, DEC: 11 };
export function tickerGameDate(ticker) {
  const mid = String(ticker || "").split("-")[1] || "";
  const m = mid.match(/^(\d{2})([A-Z]{3})(\d{2})/);
  if (!m || MON[m[2]] == null) return NaN;
  return Date.UTC(2000 + +m[1], MON[m[2]], +m[3]);
}
const DAYMS = 86400000;
const utcDay = t => { const d = new Date(t); return Number.isNaN(d.getTime()) ? NaN : Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()); };

// Kalshi's single-game series (KXMLBGAME / KXNFLGAME / KX<LEAGUE>GAME) list ONE
// "<Team> wins" YES market per side. Group the two markets for a game, require
// BOTH teams to appear, and among all such groups pick the one whose ticker date
// is closest to the Odds kickoff (within `maxDayGap` days — a matchup recurs
// across a series, so the date disambiguates). away/home resolve by relative name
// score. Returns { away, home, awayMarket, homeMarket, dayGap } (0–1) or null.
export function matchGameTwoSided(game, markets, { maxDayGap = 1 } = {}) {
  const groups = new Map();
  for (const m of markets || []) {
    const ev = m.event_ticker || String(m.ticker || "").replace(/-[^-]*$/, "");
    if (!groups.has(ev)) groups.set(ev, []);
    groups.get(ev).push(m);
  }
  const gDay = utcDay(Date.parse(game.commence_time));
  let best = null, bestGap = Infinity;
  for (const [, group] of groups) {
    let awayM = null, homeM = null, aBest = 0, hBest = 0;
    for (const m of group) {
      const txt = m.yes_sub_title || m.title || "";
      const a = nameScore(game.away_team, txt), h = nameScore(game.home_team, txt);
      if (a >= 1 && a > aBest) { aBest = a; awayM = m; }
      if (h >= 1 && h > hBest) { hBest = h; homeM = m; }
    }
    if (!(awayM && homeM && awayM !== homeM)) continue;
    const kDay = tickerGameDate(awayM.ticker) || tickerGameDate(homeM.ticker);
    const gap = Number.isFinite(gDay) && Number.isFinite(kDay) ? Math.abs(kDay - gDay) / DAYMS : 0;
    if (gap <= maxDayGap && gap < bestGap) { bestGap = gap; best = { awayM, homeM }; }
  }
  if (!best) return null;
  return { away: dollarsBlock(best.awayM), home: dollarsBlock(best.homeM), awayMarket: best.awayM, homeMarket: best.homeM, dayGap: bestGap };
}
