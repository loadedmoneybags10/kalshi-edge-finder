...
// "<Team> wins" YES market per side. Group the two markets for a game and return
// each side's own YES orderbook. away/home are disambiguated by relative name
// score WITHIN the matched event, so short names ("New York Y") still resolve.
// Returns { away, home, awayMarket, homeMarket } (0–1 prices) or null.
export function matchGameTwoSided(game, markets, { dateWindowMs = 36 * 3600 * 1000 } = {}) {
  ...
  const gt = ms(game.commence_time);
  for (const [, group] of groups) {
    const ct = ms(group[0]?.close_time);
    if (Number.isFinite(gt) && Number.isFinite(ct) && Math.abs(ct - gt) > dateWindowMs) continue;
    ...
    if (awayM && homeM && awayM !== homeM)
      return { away: dollarsBlock(awayM), home: dollarsBlock(homeM), awayMarket: awayM, homeMarket: homeM };
  }
  return null;
}
