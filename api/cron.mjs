...
export default async function handler(req, res) {
  try {
    ...
    const grade = await runGrade(sport);      // 1) settle games that finished
    const close = await runClose({});         // 2) snapshot closing lines (CLV)
    const poll = await runPoll(sport);        // 3) place today's new edges
